...
class ICNet(nn.Cell):
    ...
    def __init__(self, nclass=19, backbone='resnet50', pretrained_path='', istraining=False):
    ...
    def construct(self, x):
        ...
        h, w = x.shape[2:]
        # sub 2
        x_sub2 = ops.interpolate(x, size=(h / 2, w / 2))
...
class PyramidPoolingModule(nn.Cell):
    ...
    def construct(self, x):
        ...
        x1 = ops.interpolate(x1, size=(height, width), align_corners=True)
        feat = feat + x1

        x2 = self.pool2(x)
        x2 = ops.interpolate(x2, size=(height, width), align_corners=True)
        feat = feat + x2

        x3 = self.pool3(x)
        x3 = ops.interpolate(x3, size=(height, width), align_corners=True)
        feat = feat + x3

        x6 = self.pool6(x)
        x6 = ops.interpolate(x6, size=(height, width), align_corners=True)
        feat = feat + x6
...
class _ICHead(nn.Cell):
    ...
    def construct(self, x_sub1, x_sub2, x_sub4):
        ...
        up_x2 = ops.interpolate(x_cff_12, size=(h1 * 2, w1 * 2), align_corners=True)
        up_x2 = self.conv_cls(up_x2)
        h2, w2 = up_x2.shape[2:]

        up_x8 = ops.interpolate(up_x2, size=(h2 * 4, w2 * 4), align_corners=True)  # scale_factor=4,
...
class CascadeFeatureFusion12(nn.Cell):
    ...
    def construct(self, x_low, x_high):
        """cff_construct"""
        h, w = x_high.shape[2:]
        x_low = ops.interpolate(x_low, size=(h, w), align_corners=True)

...
class ICNetLoss(nn.Cell):
    ...
    def construct(self, *inputs):
        ...
        target_sub4 = ops.interpolate(target, size=(h / 4, w / 4)).squeeze(1)

        target_sub8 = ops.interpolate(target, size=(h / 8, w / 8)).squeeze(1)

        target_sub16 = ops.interpolate(target, size=(h / 16, w / 16)).squeeze(1)

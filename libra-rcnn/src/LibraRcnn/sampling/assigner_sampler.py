...
class AssignerSampler(nn.Cell):
    ...
    def prepare(
            self, gt_bboxes_i: ms.Tensor, valid_mask: ms.Tensor,
            bboxes: ms.Tensor, gt_valids: ms.Tensor
    ...
    def get_result(
        ...
        Args:
            assigned_gt_inds (ms.Tensor): Assidning result.
            bboxes (ms.Tensor): Proposed bboxes.
        ...
        if self.rcnn_mode:
            ...
            total_labels = ops.concat(
                (ops.cast(pos_gt_labels, ms.int32), self.sampler.labels_neg_mask)
            )
            ...
            output = total_bboxes, total_deltas, total_labels, total_mask
        else:
    ...
    def construct(
        ...
        Args:
            ...
            valid_mask (ms.Tensor): Mask that shows valid bboxes.
            bboxes (ms.Tensor): Proposed bboxes.
            gt_valids (ms.Tensor): Mask that shows valid GT bboxes

        Returns:
            Tuple: Result of sampling for training of RCNN or RPN.
        """
        gt_bboxes_i, bboxes = self.prepare(
            gt_bboxes_i, valid_mask, bboxes, gt_valids
        )
        (
            assigned_gt_inds, overlaps, max_overlaps, assigned_labels
        ) = self.assigner(
            gt_bboxes_i, gt_labels_i, valid_mask, bboxes
        )
        ...
        pos_index, pos_valid_index, neg_index, neg_valid_index = self.sampler(
            assigned_gt_inds, overlaps, max_overlaps, assigned_labels,
            gt_bboxes_i, gt_labels_i, valid_mask, bboxes, gt_valids
        )

        return self.get_result(
            assigned_gt_inds, bboxes, gt_labels_i, gt_bboxes_i, pos_index,
            pos_valid_index, neg_index, neg_valid_index
        )

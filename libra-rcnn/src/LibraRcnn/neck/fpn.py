...
class FPN(nn.Cell):
    ...
    Args:
        ...
        no_norm_on_lateral (bool): Whether to apply norm on lateral.
            Defaults to False.
        conv_cfg (Dict): Config dict for
            convolution layer. Defaults to None.
        norm_cfg (Dict): Config dict for
            normalization layer. Defaults to None.
        act_cfg (Dict): Config dict for
    ...
    def __init__(
        ...
        for shape in self.feature_shapes:
            self.interpolations.append(ms.ops.ResizeNearestNeighbor(shape))

        self.maxpool = ms.ops.MaxPool(kernel_size=1, strides=2)

    def construct(self, inputs: Union[List[ms.Tensor], Tuple]) -> Tuple:
        """Forward function."""
        ...
        for i in range(used_backbone_levels - 1, -1, -1):
            if i == used_backbone_levels - 1:
                ...
                laterals = (
                    [
                        laterals_[i] +
                        self.interpolations[i](laterals[0]),
                    ] + laterals
                )
        # build outputs
            ...
            # add conv layers on top of original feature maps (RetinaNet)
                ...
                for i in range(used_backbone_levels + 1, self.num_outs):
                    if self.relu_before_extra_convs:
                        outs.append(self.fpn_convs[i](ms.ops.relu(outs[-1])))
                    else:

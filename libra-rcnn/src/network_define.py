...
# ============================================================================
"""LibraRcnn training network wrapper."""
...
class LossNet(nn.Cell):
    """LibraRcnn loss method"""

    def construct(
            self, rpn_loss, rcnn_loss, rpn_cls_loss, rpn_reg_loss,
...
class WithLossCell(nn.Cell):
    ...
    def construct(self, x, img_shape, gt_bboxe, gt_label, gt_num):
        (
            ...
            rcnn_reg_loss
        ) = self._backbone(x, img_shape, gt_bboxe, gt_label, gt_num)
...
class TrainOneStepCell(nn.TrainOneStepWithLossScaleCell):
    ...
    def construct(self, x, img_shape, gt_bboxes, gt_label, gt_num):
        weights = self.weights
        loss = self.network(x, img_shape, gt_bboxes, gt_label, gt_num)
        scaling_sens = self.scale_sense

        _, scaling_sens = self.start_overflow_check(loss, scaling_sens)

        scaling_sens_filled = ops.ones_like(loss) * ops.cast(scaling_sens, ops.dtype(loss))
        grads = self.grad(self.network, weights)(x, img_shape, gt_bboxes, gt_label, gt_num, scaling_sens_filled)

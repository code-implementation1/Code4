...
# ============================================================================
"""Train LibraRcnn and get checkpoint files."""
import os
import shutil
import subprocess
import sys
...
from src.mlflow_funcs import (
    mlflow_log_state, mlflow_log_args,  # mlflow_decorator,
)
from src.eval_utils import (
    EvalCell, MeanAveragePrecision, COCOMeanAveragePrecision
)
...
def load_ckpt_to_network(net, checkpoint_path, finetune=False):
    ...
    if finetune:
        param_not_load = [
            'learning_rate', 'stat.bbox_head.fc_cls.weight',
            'stat.bbox_head.fc_cls.bias', 'stat.bbox_head.fc_reg.weight',
            'stat.bbox_head.fc_reg.bias', 'bbox_head.fc_cls.weight',
            'bbox_head.fc_cls.bias', 'bbox_head.fc_reg.weight',
            'bbox_head.fc_reg.bias', 'accum.bbox_head.fc_cls.weight',
            'accum.bbox_head.fc_cls.bias', 'accum.bbox_head.fc_reg.weight',
...
def main():
    ...
    experiment_name = '_'.join(
        [datetime.now().strftime('%y%m%d'), 'LibraRCNN']
    )
    ...
    eval_metrics = {
        'loss': metrics.Loss()  # dummy loss
    }
    if config.val_data_type == 'coco':
        eval_metrics['mAP'] = MeanAveragePrecision(
            num_classes=config.num_classes - 1,
            norm_pred=False, norm_gt=True
        )
    elif config.val_data_type == 'mindrecord':
        eval_metrics['mAP'] = COCOMeanAveragePrecision(
    ...
    cb = get_callbacks(
        arch='LibraRCNN', logs_dir=ckpt_save_dir / 'logs',
...
if __name__ == '__main__':
    ...
    config_ = get_config()
    ms.set_context(
        mode=ms.GRAPH_MODE, device_target=config_.device_target,

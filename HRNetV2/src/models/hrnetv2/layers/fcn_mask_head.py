...
from .conv_module import ConvModule, build_upsample_layer
from .single_roi_extractor import ROIAlign
...
class FCNMaskHead(nn.Cell):
    ...
    def __init__(
        ...
        self.max_training_num = int(
            train_cfg.rcnn.sampler.pos_fraction * train_cfg.rcnn.sampler.num
        )
        ...
        self.bboxes_range_train = ms.Tensor(
            np.arange(self.max_training_num * train_batch_size).reshape(-1, 1),
            ms.int32
        )
    ...
    def loss(self, seg_logits, labels, weights, seg_targets):
        ...
        seg_mask_loss = (
            seg_mask_loss / (ops.reduce_sum(weights, (0,)) + self.eps)
        )
        seg_mask_loss = ops.reduce_sum(seg_mask_loss, (0,))
        seg_mask_loss = seg_mask_loss * self.loss_mask_weight
        return seg_mask_loss

    def get_targets(self, bboxes_tuple, ids_tuple, gt_masks_tuple,
                    mask_divider=1.0):
        """Compute mask target for each positive proposal in the image."""
        targets_tuple = []
        for i in range(self.train_batch_size):
            bboxes = bboxes_tuple[i]
            ids = ids_tuple[i]
            gt_masks = gt_masks_tuple[i]

            _, maxh, maxw = gt_masks.shape
            bboxes = bboxes / mask_divider
            x1 = ops.clip(bboxes[:, [0]], 0, maxw)
            x2 = ops.clip(bboxes[:, [2]], 0, maxw)
            y1 = ops.clip(bboxes[:, [1]], 0, maxh)
            y2 = ops.clip(bboxes[:, [3]], 0, maxh)
            rois = ops.concat(
                (ops.cast(ids, ms.float32), x1, y1, x2, y2), axis=1
            )
    ...
    def choose_masks(self, mask_logits, det_labels):
        ...
        if self.training:
            pred_masks = ops.reshape(
                pred_masks, (-1, self.mask_size[0], self.mask_size[1])
            )
        else:
            pred_masks = ops.reshape(
                pred_masks,
    ...
    @staticmethod
    def _do_paste_mask(masks, boxes, img_h, img_w):
        ...
        Returns:
            tuple: (Tensor, tuple). The first item is mask tensor,
                the second one is the slice object.

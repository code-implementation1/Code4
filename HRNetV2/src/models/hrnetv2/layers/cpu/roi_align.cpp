...
template <typename T>
void roi_align_cpu(const int nthreads, const T* input, const T* rois,
                   ...
                   const int* aligned_, const int channels, const int height,
                   const int width) {
      ...
      for (int ph = 0; ph < pooled_height; ph++) {
        for (int pw = 0; pw < pooled_width; pw++) {
          ...
          for (int iy = 0; iy < roi_bin_grid_h; iy++) {
            const T y = roi_start_h + ph * bin_size_h + static_cast<T>(iy + .5f) *
                            bin_size_h / static_cast<T>(roi_bin_grid_h);
            for (int ix = 0; ix < roi_bin_grid_w; ix++) {
              const T x = roi_start_w + pw * bin_size_w + static_cast<T>(ix + .5f) *
                              bin_size_w / static_cast<T>(roi_bin_grid_w);
              PreCalc<T> pc = pre_calc[pre_calc_index];
              T val = pc.w1 * offset_input[pc.pos1] + pc.w2 * offset_input[pc.pos2] +
...
template <typename T>
void check_roi_size(T& roi_width, T& roi_height, int aligned) {
  if (aligned) {
    assert(roi_width >= 0 && roi_height >= 0 &&
...
template <typename T>
void roi_align_backward(const int nthreads, const T* grad_output, const T* rois,
  ...
  for (int index = 0; index < nthreads; index++) {
    ...
    T roi_width = roi_end_w - roi_start_w;
    T roi_height = roi_end_h - roi_start_h;
    check_roi_size(roi_width, roi_height, aligned);
    T bin_size_h = static_cast<T>(roi_height) / static_cast<T>(pooled_height);
    T bin_size_w = static_cast<T>(roi_width) / static_cast<T>(pooled_width);
    ...
    if (abs(grad_output_this_bin) < 1e-10) { continue; }
    if (pool_mode == 0) {
      // We do max pooling inside a bin
      ...
      for (int iy = 0; iy < roi_bin_grid_h; iy++) {
        const T y = roi_start_h + ph * bin_size_h + static_cast<T>(iy + .5f) *
                        bin_size_h / static_cast<T>(roi_bin_grid_h);  // e.g., 0.5, 1.5
        for (int ix = 0; ix < roi_bin_grid_w; ix++) {
          const T x = roi_start_w + pw * bin_size_w + static_cast<T>(ix + .5f) *
                          bin_size_w / static_cast<T>(roi_bin_grid_w);
          T w1, w2, w3, w4;
...
template <typename T>
void fill_zeros(const int n, T* grads) {
    for (int i = 0; i < n; i++) {
        grads[i] = 0.0;
    }
}

int get_stride(int64_t **shapes, int *ndims, int idx, int shape_idx_start) {
  int res = 1;
  for (int i = shape_idx_start; i < ndims[idx]; i++) {
    res *= shapes[idx][i];
  }
  return res;
}
...
extern "C" int CustomROIAlignBackward(
  ...
  int grad_output_idx = 4;
  int grad_input_idx = 11;

  int grad_output_size = 1;
  int grad_input_size = 1;
  int channels = shapes[grad_input_idx][1];
  int height = shapes[grad_input_idx][2];
  int width = shapes[grad_input_idx][3];

  int n_stride = get_stride(shapes, ndims, grad_output_idx, 1);
  int c_stride = get_stride(shapes, ndims, grad_output_idx, 2);
  int h_stride = get_stride(shapes, ndims, grad_output_idx, 3);
  int w_stride = get_stride(shapes, ndims, grad_output_idx, 4);

  for (int i = 0; i < ndims[grad_output_idx]; i++) {
    grad_output_size *= shapes[grad_output_idx][i];
  }
  for (int i = 0; i < ndims[grad_input_idx]; i++) {
    grad_input_size *= shapes[grad_input_idx][i];
  }

  fill_zeros(grad_input_size, static_cast<float *>(grad_input));

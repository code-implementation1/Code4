    ...
    def __init__(
        ...
        self.forward_op = ops.Custom(
            os.path.join(
                ...
                f'roi_align_{self.device.lower()}.so:CustomROIAlign'
            ),
            out_shape=self._get_out_shape,
            out_dtype=self._get_out_type,
            bprop=self.backward(self.device),
            func_type='aot',
            reg_info=reg_info.target(self.device).get_op_info()
        )

    def backward(self, device='GPU'):
        """Prepare backward."""
        ...
        op_path = f'roi_align_{device.lower()}.so:CustomROIAlignBackward'
        ...
        backward_op = ops.Custom(
            ...
            func_type="aot",
            reg_info=reg_info.target(device).get_op_info()
        )

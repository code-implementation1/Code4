...
def call_max_pool_2d(x, kernel_size: int = 1,
                     stride: int = 1,
                     pad_mode: str = "valid",
                     data_format: str = "NCHW"):
    maxpool = nn.MaxPool2d(kernel_size, stride, pad_mode, data_format)
    return maxpool(x)
...
class HRFPN(nn.Cell):
    ...
    def __init__(self,
        ...
        if pooling_type == 'MAX':
            self.pooling = ops.max_pool2d
        else:
            self.pooling = ops.avg_pool2d
    ...
    def construct(self, inputs):
        ...
        for i in range(1, self.num_ins):
            outs.append(ops.interpolate(inputs[i], mode='bilinear',
                                        scale_factor=2.0**i,

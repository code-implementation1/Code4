...
# ============================================================================
"""DetectEval that is used to analyze predictions."""
import csv
import os
import logging
import warnings
...
class DetectEval(COCOeval):
    ...
    def evaluate_every_class(self):
        ...
        """
        logging.info("Evaluate every class's predision and recall")
        p = self.params
        cat_ids = p.catIds
        labels = p.labels
        result = []
    ...
    def plot_pr_curve(self, eval_result_path):
        ...
        """
        logging.info("Plot pr curve about every class")
    ...
    def save_images(self, config, eval_result_path, iou_threshold=0.5):
        ...
        """
        logging.info("Saving images of ok ng")
        p = self.params
        img_ids = p.imgIds
        ...
        for img_id in img_ids:
            ...
            if config.dataset == "coco":
                im_path_dir = os.path.join(config.val_dataset, 'data')
            ...
            if not precision_ng and not recall_ng:
                ...
                self._draw_boxes(image, dts, img_id, labels)
    ...
    def _draw_boxes(self, image, dts, img_id, labels):
        for obj in dts:
            _id = obj["image_id"]
            if _id == img_id:
                bbox = obj["bbox"]
                ...
                draw_one_box(image, label, (xmin, ymin, xmax, ymax),
                             category_id)

    def compute_precison_recall_f1(self, min_score=0.1):
        logging.info('Compute precision, recall, f1...')
    ...
    def compute_tp_fp_confidence(self):
        logging.info('Compute tp and fp confidences')
        if not self.evalImgs:
            logging.info('Please run evaluate() first')
        p = self.params
        catIds = p.catIds if p.useCats == 1 else [-1]
            ...
            for a0 in a_list:
                ...
                for maxDet in m_list:
                    ...
                    if label not in correct_conf_dict.keys():
                        ...
                        logging.info('maxDet: %d areagRng: %d', maxDet, p.areagRng)
                        break
    ...
    def write_best_confidence_threshold(self, cat_pr_dict, cat_pr_dict_origin, eval_result_path):
        ...
        """
        logging.info("Write best confidence threshold to csv")

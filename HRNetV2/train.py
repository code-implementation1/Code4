...
def load_ckpt_to_network(net, checkpoint_path, finetune=False):
    ...
    if finetune:
        param_not_load = [
            ...
            'zeros_.bbox_head.fc_cls.weight',
            'zeros_.bbox_head.fc_cls.bias',
            'zeros_.bbox_head.fc_reg.weight',
            'zeros_.bbox_head.fc_reg.bias',
            'accumulate_.bbox_head.fc_cls.weight',
            'accumulate_.bbox_head.fc_cls.bias',
            'accumulate_.bbox_head.fc_reg.weight',
            'accumulate_.bbox_head.fc_reg.bias',
            ...
            'zeros_.mask_head.conv_logits.bias',
            'zeros_.mask_head.conv_logits.weight',
            'accumulate_.mask_head.conv_logits.bias',
            'accumulate_.mask_head.conv_logits.weight',

            'stat.mask_iou_head.fc_mask_iou.bias',
            'stat.mask_iou_head.fc_mask_iou.weight',
            'mask_iou_head.fc_mask_iou.weight',
            'mask_iou_head.fc_mask_iou.bias',
            'accum.mask_iou_head.fc_mask_iou.weight',
            'accum.mask_iou_head.fc_mask_iou.bias',
            'zeros_.mask_iou_head.fc_mask_iou.bias',
            'zeros_.mask_iou_head.fc_mask_iou.weight',
            'accumulate_.mask_iou_head.fc_mask_iou.bias',
            'accumulate_.mask_iou_head.fc_mask_iou.weight',
        ]
    else:
        param_not_load = ['learning_rate', 'global_step']
...
def main():
    ...
    eval_metrics = get_metrics(config, net)
    ...
    model.fit(
        config.epoch_size, train_dataset, valid_dataset=val_dataset,
        callbacks=cb, dataset_sink_mode=bool(config.datasink),

...
from .mask_assigner_sampler import MaskAssignerSampler
...
def build_mask_max_iou_random(
    ...
    """Build MaxIOUAssigner and RandomSampler for MaskRCNN."""
    assigner = MaxIOUAssigner(
        num_bboxes=num_bboxes, num_gts=num_gts, **assigner_cfg
    )
    sampler = RandomSampler(
    ...
    assigner_sampler_obj = MaskAssignerSampler(
        assigner=assigner, sampler=sampler, rcnn_mode=rcnn_mode,
        bbox_coder=bbox_coder
    )
    return assigner_sampler_obj
...
__all__ = [
    'MaxIOUAssigner', 'RandomSampler', 'AssignerSampler',
    'build_max_iou_random', 'build_mask_max_iou_random'
]

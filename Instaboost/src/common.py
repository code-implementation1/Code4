...
def get_callbacks(
    ...
    best_ckpt_save_cb_bbox = EvalCocoCallback(
        best_ckpt_path=best_ckpt_dir, buffer=best_ckpt_num, prefix=prefix,
        metric='bbox_mAP'
    )
    best_ckpt_save_cb_seg = EvalCocoCallback(
        best_ckpt_path=best_ckpt_dir, buffer=best_ckpt_num, prefix=prefix,
        metric='seg_mAP'
    )
    ...
    return [
        ...
        eval_time_cb,
        ckpoint_cb,
        best_ckpt_save_cb_bbox,
        best_ckpt_save_cb_seg,
...
def set_context(config):
    ms.set_context(
        ...
        mode=(
            ms.context.GRAPH_MODE if config.mode == 'graph'

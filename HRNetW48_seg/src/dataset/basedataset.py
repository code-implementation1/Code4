...
class BaseDataset:
    ...
    def inference(self, model, image, flip=False):
        ...
        shape = image.shape
        pred = model(image)
        pred = P.ResizeBilinearV2()(pred, (shape[-2], shape[-1]))
        if flip:
            flip_img = image.asnumpy()[:, :, :, ::-1]
            flip_output = model(Tensor(flip_img.copy()))
            flip_output = P.ResizeBilinearV2()(flip_output, (shape[-2], shape[-1]))
    ...
    def multi_scale_inference(self, model, image, scales=None, flip=False):
        ...
        for scale in scales:
            ...
            preds = P.ResizeBilinearV2()(preds, (ori_height, ori_width))
